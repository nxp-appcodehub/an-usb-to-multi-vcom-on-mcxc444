...
/* CDC ACM information */
USB_DMA_INIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE) static usb_cdc_acm_info_t s_usbCdcAcmInfo[] = {
...
uint8_t g_CdcVcomDicBulkInEndpoint[USB_DEVICE_CONFIG_CDC_ACM];

/* CDC VCOM DIC Bulk OUT endpoint number*/
uint8_t g_CdcVcomDicBulkOutEndpoint[USB_DEVICE_CONFIG_CDC_ACM];

/* CDC VCOM CIC Interrupt IN endpoint number*/
uint8_t g_CdcVcomCicInterruptInEndpoint[USB_DEVICE_CONFIG_CDC_ACM];


/* CDC VCOM DIC interface index */
uint8_t g_CdcVcomDicInterfaceIndex[USB_DEVICE_CONFIG_CDC_ACM];

/* CDC VCOM CIC interface index */
uint8_t g_CdcVcomCicInterfaceIndex[USB_DEVICE_CONFIG_CDC_ACM];
...
usb_status_t USB_DeviceCdcVcomSetConfigure(usb_device_handle handle, uint8_t configure)
{
    ...
    if (g_deviceComposite->currentConfiguration)
    {
        for (uint8_t i = 0; i < USB_DEVICE_CONFIG_CDC_ACM; i++)
        {
            ...
            USB_DeviceDeinitEndpoint(g_deviceComposite->deviceHandle,
                                     ((g_deviceComposite->cdcVcom[i].bulkInEndpoint) |
                                      (USB_IN << USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT)));
            USB_DeviceDeinitEndpoint(g_deviceComposite->deviceHandle,
    ...
    if (USB_COMPOSITE_CONFIGURE_INDEX == configure)
    {
        for (uint8_t i = 0; i < USB_DEVICE_CONFIG_CDC_ACM; i++)
        {
			...
			epCallback.callbackParam = (void *)&g_deviceComposite->cdcVcom[i].communicationInterfaceNumber;

			epInitStruct.zlt = 0;
			epInitStruct.transferType = USB_ENDPOINT_INTERRUPT;
			epInitStruct.endpointAddress =
					g_CdcVcomCicInterruptInEndpoint[i] | (USB_IN << USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT);


			epInitStruct.maxPacketSize = FS_CDC_VCOM_INTERRUPT_IN_PACKET_SIZE;
			epInitStruct.interval = FS_CDC_VCOM_INTERRUPT_IN_INTERVAL;

			g_deviceComposite->cdcVcom[i].interruptEndpoint = g_CdcVcomCicInterruptInEndpoint[i];
			g_deviceComposite->cdcVcom[i].interruptEndpointMaxPacketSize = epInitStruct.maxPacketSize;
			g_deviceComposite->cdcVcom[i].communicationInterfaceNumber = g_CdcVcomCicInterfaceIndex[i];
			...
			epCallback.callbackParam = (void *)&g_deviceComposite->cdcVcom[i].dataInterfaceNumber;

			epInitStruct.zlt = 0;
			epInitStruct.interval = 0;
			epInitStruct.transferType = USB_ENDPOINT_BULK;
			epInitStruct.endpointAddress =
					g_CdcVcomDicBulkInEndpoint[i] | (USB_IN << USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT);


			epInitStruct.maxPacketSize = FS_CDC_VCOM_BULK_IN_PACKET_SIZE;

			g_deviceComposite->cdcVcom[i].bulkInEndpoint = g_CdcVcomDicBulkInEndpoint[i];
			g_deviceComposite->cdcVcom[i].bulkInEndpointMaxPacketSize = epInitStruct.maxPacketSize;
			USB_DeviceInitEndpoint(handle, &epInitStruct, &epCallback);
			...
			epInitStruct.transferType = USB_ENDPOINT_BULK;
			epInitStruct.endpointAddress =
					g_CdcVcomDicBulkOutEndpoint[i] | (USB_OUT << USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT);

			epInitStruct.maxPacketSize = FS_CDC_VCOM_BULK_OUT_PACKET_SIZE;

			g_deviceComposite->cdcVcom[i].bulkOutEndpoint = g_CdcVcomDicBulkOutEndpoint[i];
			g_deviceComposite->cdcVcom[i].bulkOutEndpointMaxPacketSize = epInitStruct.maxPacketSize;
			USB_DeviceInitEndpoint(handle, &epInitStruct, &epCallback);

			g_deviceComposite->cdcVcom[i].dataInterfaceNumber = g_CdcVcomDicInterfaceIndex[i];

			/* Schedule buffer for receive */
			USB_DeviceRecvRequest(handle, g_deviceComposite->cdcVcom[i].bulkOutEndpoint,
								  g_deviceComposite->cdcVcom[i].currRecvBuf,
...
usb_status_t USB_DeviceCdcVcomInit(usb_device_composite_struct_t *deviceComposite)
{
    ...
    for (uint8_t i = 0; i < USB_DEVICE_CONFIG_CDC_ACM; i++)
    {
        g_deviceComposite->cdcVcom[i].lineCoding = (uint8_t *)&s_lineCoding;
        g_deviceComposite->cdcVcom[i].abstractState = (uint8_t *)&s_abstractState;
        g_deviceComposite->cdcVcom[i].countryCode = (uint8_t *)&s_countryCode;
        g_deviceComposite->cdcVcom[i].usbCdcAcmInfo = &s_usbCdcAcmInfo;
